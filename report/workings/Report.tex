...
The Schur Preconditioner is not fully parallelizable. Further improvments could be made to determining $A^{-1}$. An example would be applying a patch preconditioning method.\\
The variable step-size method is servicable but can be considerably improved. There are a number of methods which from local conditions of the function could more efficiently determine an appropriate step-size.\\
Similarly the time-stepping method was designed exclusively for the stability test. For this purpose it works very well. However various improvements could be made for accuracy and speed. For instance we could follow up on the Picards iterations with a few Newton iterations. The Picards solver would have presumably brought us into the radius of convergence.\\
