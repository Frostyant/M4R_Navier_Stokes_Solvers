...
\begin{equation}
\begin{aligned}
	u_x & =  -cos(2 \pi x) sin(2 \pi y)\\
	u_y & =  sin(2 \pi x) cos (2 \pi y)\\
	p & = 0\\
	F_x & =  - 4 \pi^2 cos(2 \pi x) cos(2 \pi y) + 4 \pi^2 sin(2 \pi x) sin(2 \pi y)\\
	F_y & =    4 \pi^2 cos(2 \pi x) cos(2 \pi y)  - 4 \pi^2 sin(2 \pi x) sin(2 \pi y)    
\end{aligned}
\end{equation}
...
When interested in the stability of a solution analytically we commonly first determine the solution. We would then consider the normal modes form of the equation before perturbing it. After some algebra we determin the most unstable mode and deduce from that.\\
...
For this we will use the manufactured solution $u_x = -x sin(2 \pi x y)$, $u_y = y sin(2 \pi x y)$ and $p = sin(x y)$.
We then add in a forcing term to ensure the above respects the stokes equation.\\
Hence we get : $F = \nabla p - \mu \nabla^2 u$.\\
Figures and show us the convergence graphs in velocity and pressure for the $H_{div}$ solver.\\
Hence we can deduce we obtain a convergence rate of a bit over $2$, as expected given the usage of the BDM2 and DG1 space.
This shows that the preconditioning and discretisation holds for the stokes equation if boundary conditions are $0$.

\subsection{Navier-Stokes Convergence}

We again use a manufactured solution to determine the convergence rate of our solver.
We can re-use the first solution in the stokes convergence test, $u_x = -x sin(2 \pi x y)$, $u_y = y sin(2 \pi x y)$ and $p = sin(x y)$. However the forcing term will now be $F = \nabla p - \mu \nabla^2 u + u \cdot \nabla u$.
