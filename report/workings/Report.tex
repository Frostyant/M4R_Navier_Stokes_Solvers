...
\end{align*}
We then cut this inegral up into a sum of integrals over each cell and integrate on each one by parts separately. Performing this integration we get for each cell :
\begin{align}
\int_e u \cdot (\nabla \times (u \times v) \ ) dx - \int_{\delta e} u^+ \cdot (n \times (u \cdot v) \ ) dS
\end{align}
Not that the second term becomes the following on the computational domain boundary :
\begin{align*}
- \int_{\delta \Omega} u_0 \cdot (n \times (u \cdot v) \ ) ds
\end{align*}
Hence the weak form of our advection term becomes : 
\begin{align}
 Adv &= \int_e u \cdot (\nabla \times (u \times v) \ ) dx - \int_{\delta e} u^+ \cdot (n \times (u \cdot v) \ ) dS \\ 
...
Given the navier stokes equations we could try to naively solve directly applying the above preconditioning and discretization methods. However at high Reynolds number this approach quickly fails unless the mesh is refined, leading to high computational cost.\\
...
We could use the stokes solution, which we can most likely solve. We then increase $c$ to $1$ and try to solve using the stokes solution as a guess. If it still fails we can reduce the step-size of $c$ and solve for $c = 0.5$ instead before solving for full advection. If necessary we can bring down the step-size multiple times until we reach a $c$ for which it works, after which we then gradually increase c using the same procedure until we solve for full advection.\\
\\
...
The second involves approximating the $(u,p)$ at a subsequent step more accuratly. Rather than just using the prior value of $(u,p)$ when $c = 1$, call it $x_c$, we use taylor expansion. We can consider $x_c$ to be a function of $c$, hence get :
$$
...
\begin{figure}[ht]
  ...
  \caption{DG2-BDM1 Stokes velocity Convergence Graph}

  \vspace*{\floatsep}

  \includegraphics[width=\linewidth]{stokes_pressure_convergence_dbc0.png}
  \caption{DG2-BDM1 Stokes velocity Convergence Grap}
\end{figure}
